		...
		if (regSocket.receive(packet) == Socket::Status::Done)
		{
			if (packet.getDataSize() > 0)
			{
				...
				if (packet >> name)
				{
					...
					cout <<"Ip of client" << newClient.Ip.toString() << endl;
					newClient.dataSocket = new UdpSocket;
					if (newClient.dataSocket->bind(Socket::AnyPort) != Socket::Status::Done)
				...
				if (packet >> port)
				{
					newClient.port = static_cast<unsigned short>(port);
					cout << "Ip of client" << newClient.port << endl;
				}
				else
				{
			...
			for (int i = 0; i < clientsVec.size() - 1; i++)
			{
				if (!clientsVec[i].done)
				{
					...
					if (clientsVec[i].sDataPacket.getDataSize() == 0){
						int numberNew = clientsVec.size() - 1;
						clientsVec[i].sDataPacket << "NEW" << clientsVec.back().name << numberNew;
					}
					cout << "Send new" << endl;
					cout << "Ip of client " << tempIp.toString() << endl;
					cout << "Port of client " << tempPort << endl;
...
Socket::Status NetworkServer::sendDedicatedDataPort()
{
	if (regStep == 3)
	{
		...
		if (packet.getDataSize() == 0)
			packet << static_cast<Uint16>(clientsVec.back().dataSocket->getLocalPort());/*<< clientsVec.size()-1;*/
...
Socket::Status NetworkServer::receiveData(unsigned int& receivedClientIndex)
{
	for (int i = 0; i < clientsVec.size(); ++i)
	{
		if (!clientsVec[i].dataSocket) continue;
		if (clientsVec[i].dataSocket->isBlocking()) clientsVec[i].dataSocket->setBlocking(false);
		IpAddress tempIp = clientsVec[i].Ip;
		unsigned short tempPort = clientsVec[i].port;
...
Socket::Status NetworkServer::sendDataToAll(Packet dataPacket)
{
	if (sendingsRateTimer.getElapsedTime().asMilliseconds() > sendingsRate)
	{
		for (auto& client : clientsVec)
		{
			if (!client.done)
			{
				if (client.dataSocket->isBlocking()) client.dataSocket->setBlocking(false);
				IpAddress tempIp = client.Ip;
				
				unsigned short tempPort = client.port;

				if (client.sDataPacket.getDataSize() == 0) client.sDataPacket = dataPacket;

				if (client.dataSocket->send(client.sDataPacket, tempIp, tempPort) == Socket::Status::Done)
				{
					client.done = true;
				}
				...
				for (const auto& client:clientsVec)
				{
					if (!client.done) allIsDone = false;
				}
